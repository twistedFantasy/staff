      ...
      <div v-for="skill in userProfile.skills" v-bind:key="skill.name">
        <div class="text-xs-center">
          <v-chip outline color="primary" close @input="deleteSkill(skill.name)">
            {{ skill.name }}
            <v-icon class="edit-icon" size="20" right>edit</v-icon>
          </v-chip>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    save() {
      const userProfile = JSON.parse(JSON.stringify(this.$store.state.user.userProfile));
      const userId = this.$store.state.user.logedUserId;
      const newProfile = { ...userProfile, skills: [ ...userProfile.skills, {name: this.newSkill} ]}
      if(this.newSkill) {
